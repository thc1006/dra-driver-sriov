...
var _ = Describe("NRI updateNetworkDeviceData ordering", func() {
	...
	It("does not update claim status when checkpoint persistence fails", func() {
		...
		// A store that refuses the device stands in for a checkpoint that cannot
		// be written; file permissions would not stop a test running as root.
		plugin.podManager = checkpointRefuses{deviceStore: pm, refuse: deviceNamed("dev-a")}
